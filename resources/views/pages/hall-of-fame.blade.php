...
@section('content')
    @php
        ...
        $awardCount = collect($awardYears)->sum(fn ($rows) => $rows->count());

        $hasTeams = $tournamentCount > 0;
        $hasPersonal = $awardCount > 0;

        // The earliest year on either side of the page, so "Since" does not ignore a
        // year that only ever produced an individual award.
        $since = collect($years)->keys()
            ->merge(collect($awardYears)->keys())
            ->filter(fn ($year) => $year !== 'Undated')
            ->min();
    ...
    <section class="hero-section relative bg-gradient-to-br from-amber-950 via-gray-900 to-black text-white pt-28 pb-14 md:pt-32 md:pb-16 overflow-hidden">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <p class="text-sm md:text-base text-gray-400 mt-5 max-w-2xl">
                Every result we have announced, newest first, in two categories: what teams won
                and what individual players were given. These figures are frozen at the moment
                the result was published, so correcting a match afterwards never rewrites what
                was announced.
            </p>

            @if ($hasTeams || $hasPersonal)
                <div class="flex flex-wrap gap-x-10 gap-y-4 mt-8">
                    ...
                    <div>
                        <span class="block text-2xl font-bold tabular-nums">{{ $awardCount }}</span>
                        <span class="text-xs font-semibold uppercase tracking-widest text-gray-400">
                            Individual {{ Str::plural('award', $awardCount) }}
                        </span>
                    </div>
                    <div>
                        <span class="block text-2xl font-bold tabular-nums">{{ $since ?? '—' }}</span>
    ...
    <section class="py-10 md:py-16 bg-gray-50">
        <div class="max-w-5xl mx-auto px-4 sm:px-6">
            ...
            {{-- Category nav. Plain anchors rather than a scripted tab strip, so it
                 works before any JavaScript runs and a link to one category can be
                 shared. Only offered when both categories have something in them. --}}
            @if ($hasTeams && $hasPersonal)
                <nav class="flex flex-wrap gap-2 mb-8" aria-label="Categories">
                    <a href="#team"
                       class="inline-flex items-center gap-2 rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:border-amber-300 hover:text-amber-700 transition">
                        <span aria-hidden="true">&#127942;</span>
                        Team champions
                        <span class="rounded bg-gray-100 px-1.5 py-0.5 text-xs tabular-nums text-gray-500">{{ $podiumCount }}</span>
                    </a>

                    <a href="#personal"
                       class="inline-flex items-center gap-2 rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:border-indigo-300 hover:text-indigo-700 transition">
                        <span aria-hidden="true">&#11088;</span>
                        Individual honours
                        <span class="rounded bg-gray-100 px-1.5 py-0.5 text-xs tabular-nums text-gray-500">{{ $awardCount }}</span>
                    </a>
                </nav>
            @endif
            ...
            {{-- ================= Team champions ================= --}}
            @if ($hasTeams)
                <div id="team" class="scroll-mt-28">
                    <div class="flex items-center gap-3 mb-1">
                        <span class="text-lg" aria-hidden="true">&#127942;</span>
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Team Champions</h2>
                    </div>
                    <p class="text-sm text-gray-500 mb-8">
                        Won on the table, over every match of the competition.
                    </p>

                    @foreach ($years as $year => $entries)
                        <div class="flex items-center gap-4 mb-6 {{ $loop->first ? '' : 'mt-12' }}">
                            <h3 class="text-xl font-bold text-gray-900">{{ $year }}</h3>
                            ...
                            <article class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
                                ...
                                <div class="flex flex-wrap items-start justify-between gap-4 px-5 md:px-7 py-5 border-b border-gray-100">
                                    <div class="min-w-0">
                                        ...
                                        <h4 class="text-lg font-bold text-gray-900">{{ $tournament?->name ?? 'Tournament' }}</h4>
                                    ...
                                    @foreach ($podium as $champion)
                                        ...
                                        <div class="relative px-5 py-7 text-center border-b sm:border-b-0 sm:border-r border-gray-100 last:border-r-0 last:border-b-0 {{ $medal[2] }}">
                                            ...
                                            <div class="flex items-center justify-center gap-2.5 mt-1.5">
                                                <x-team-crest :registration="$champion->entrant?->registration"
                                            ...
                                            @if ((float) $champion->total_points > 0)
                                                <p class="text-sm text-gray-500 mt-1.5 tabular-nums">
            ...
            {{-- ================= Individual honours ================= --}}
            @if ($hasPersonal)
                <div id="personal" class="scroll-mt-28 {{ $hasTeams ? 'mt-16 pt-12 border-t border-gray-200' : '' }}">
                    <div class="flex items-center gap-3 mb-1">
                        <span class="text-lg" aria-hidden="true">&#11088;</span>
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Individual Honours</h2>
                    </div>
                    <p class="text-sm text-gray-500 mb-8">
                        Given to one player for their own figures, by the name they play under.
                    </p>

                    @foreach ($awardYears as $year => $rows)
                        <div class="flex items-center gap-4 mb-6 {{ $loop->first ? '' : 'mt-12' }}">
                            <h3 class="text-xl font-bold text-gray-900">{{ $year }}</h3>
                            <span class="grow h-px bg-gray-200"></span>
                            <span class="text-xs font-semibold text-gray-400 shrink-0">
                                {{ $rows->count() }} {{ Str::plural('award', $rows->count()) }}
                            </span>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            @foreach ($rows as $award)
                                <article class="rounded-xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white px-4 py-4">
                                    ...
                                    @if ($award->event_participant_id)
                                        <a href="{{ route('player', $award->event_participant_id) }}"
                                           class="block text-base font-bold text-gray-900 mt-1.5 hover:text-indigo-700 hover:underline transition">
                                            {{ $award->display_name }}
                                        </a>
                                    @else
                                        <p class="text-base font-bold text-gray-900 mt-1.5">{{ $award->display_name }}</p>
                                    @endif
            ...
            {{-- Neither category has anything in it. One message rather than two empty
                 headings, which would read as a page that is broken. --}}
            @unless ($hasTeams || $hasPersonal)
