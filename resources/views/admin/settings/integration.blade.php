            ...
            <div class="mt-5">
                <x-admin.panel title="Connect The Page" icon="lock">
                    <div class="px-5 py-4">
                        ...
                        {{-- This panel's own result, on its own session key. Sharing one key
                             with the check below put the answer to a press here in the box
                             under the other button. --}}
                        @if (session('connect_facebook_success'))
                            <div role="status" class="flex items-start gap-3 bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                                <svg class="w-5 h-5 shrink-0 text-green-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <div class="text-sm text-green-800">
                                    <p class="font-semibold mb-0.5">Page connected</p>
                                    <p>{{ session('connect_facebook_success') }}</p>
                                </div>
                            </div>
                        @endif

                        @if (session('connect_facebook_error'))
                            <div role="alert" class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                                <div class="flex items-start gap-3">
                                    <svg class="w-5 h-5 shrink-0 text-red-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-red-900 mb-1">Could not connect</p>
                                        <pre class="text-xs text-red-800 whitespace-pre-wrap break-words font-mono bg-red-100/60 rounded p-2">{{ session('connect_facebook_error') }}</pre>
                                        <p class="text-xs text-red-700 mt-2">
                                            Nothing was changed. The token that was already saved is still there.
                                        </p>
                                    </div>
                                </div>
                            </div>
